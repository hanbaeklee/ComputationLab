...
mTransMmu   = [0.9000, 0.1000; ...
               0.1000, 0.9000];
vGridMmu    = [0.80, 1.20]'*pMmu;

% Combine the shocks
vGridAOrig  = vGridA;
vGridMmuOrig = vGridMmu;
mTransAOrig = mTransA;
mTransMmuOrig = mTransMmu;

vGridA      = kron(vGridA,ones(pNumGridMmu,1));
vGridMmu    = kron(ones(pNumGridA,1),vGridMmu);
pNumGridA   = pNumGridA * pNumGridMmu;
mTransA     = kron(mTransA,mTransMmu);
...
vr         = pAalpha*Aprime*vK.^(pAalpha-1) + (1-pDdelta) + (vMmu/2).*(vKprime./vK).^2 - (vMmu/2)*(1-pDdelta)^2;
vY         = vA.*vK.^(pAalpha);
vI         = vY - tempC - (vMmu/2).*(vKprime./vK - (1-pDdelta)).^2.*vK;
vJnew      = vA.*vK.^(pAalpha) - vI - (vMmu/2).*(vKprime./vK - (1-pDdelta)).^2.*vK + tempV2;
...
vCnew = vGridA(vSimPath).*vKnew.^(pAalpha) - vI - (vMmu/2).*(vKprimenew./vKnew - (1-pDdelta)).^2.*vKnew;
...
exoState1 = vGridA(vSimPath(BURNIN+1:end-1));
exoState2 = vGridMmu(vSimPath(BURNIN+1:end-1));
...
exoStateTemp1 = exoState1(iTrans);
exoStateTemp2 = exoState2(iTrans);
tempX = [1 ...
    ,log(endoStateTemp)...
    ,log(exoStateTemp1)...
    ,log(exoStateTemp2)...
%     ,log(endoStateTemp)*log(exoStateTemp1)...
