...
% before.
% load '../solutions/WIP_kt2008_bc.mat';
...
for itrans = pathlength:-1:1
    ...
    kprimeopt       = fnOptInv_matrix(pbeta,pdelta,pgamma_i,vgridk,p,tj0);
    ...
    tj0_trans       = tj0';
    indxlow         = sub2ind(size(tj0_trans),(1:pnumgridz)',templocation');
    indxhigh        = sub2ind(size(tj0_trans),(1:pnumgridz)',(templocation+1)');
    
    tempfutureval   = tj0_trans(indxlow).*weightdown' +...
                      tj0_trans(indxhigh).*(1 - weightdown');
    
    futureval       =  - pgamma_i*kprimeopt*p ...
                    + pbeta*tempfutureval';
    ...
    futurevalconst  = - pgamma_i*kprimeconst*p ...
                    + pbeta*tempfutureval ;
    ...
    % update
    tj1(:,:,itrans)       = optval;
    tpoleffdvd(:,:,itrans) = z.*k.^(palpha).*l.^(pgamma) - (tempI).*(xistar/pfc)...
                             - (tempIconst).*((pfc-xistar)/pfc);
    tpoli(:,:,itrans)     = pgamma_i*kprimeopt-(1-pdelta)*k;
    tpolic(:,:,itrans)    = pgamma_i*kprimeconst-(1-pdelta)*k;
...
for iz = 1:pnumgridz
    ...
    % policies
    nextkall = tpolk(1,iz,itrans); % note that the target capital is independent of current capital.
...
% save (mid)
%=========================  
% save '../solutions/WIP_kt2008_bc.mat';
...
hold off;
box off;
legend("Predicted","Realized","Khan and Thomas (2008)","location","northeast");
...
location = ['../figures/rtm_ktcomp.pdf'];
saveas(gcf, location);


%%
%=========================  
% fitting LoM of p into the linear specification 
%========================= 
endostate = tK(burnin+1:end-2);
exostate = vgridA(tsimpath(burnin+1:end-1));
endostateprime = tp(burnin+1:end-1);
intercept = ones(size(endostate));
...
samplePeriod = 500:800;
figure;
plot(samplePeriod,endostateprime(samplePeriod),'Color','red','LineWidth',1.5);hold on;
plot(samplePeriod,recovered(samplePeriod),'Color','blue','LineStyle','--','LineWidth',1.5);
legend("True LoM","Linear LoM","location","best","FontSize",15)
...
%%
% plot - for report
predictedpath = tp(burnin+1:end-2);
realizedpath = eq.p(burnin+1:end-2);
...
hold off;
box off;
% legend("Predicted","Realized","Khan and Thomas (2008)","location","southeast");
ylabel("Marginal utility","FontSize",15);
xlabel("Time (year)","FontSize",15);
ax = gca;
ax.FontSize = 15; 
set(gcf, 'PaperPosition', [0 0 6 5]); %Position plot at left hand corner with width a and height b.
set(gcf, 'PaperSize', [6 5]); %Set the paper to have width a and height b.Grid off;
location = ['../figures/rtm_ktcomp_p.pdf'];
saveas(gcf, location);
